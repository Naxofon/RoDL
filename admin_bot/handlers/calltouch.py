...
PREFECT_DEPLOYMENT_CALLTOUCH = settings.PREFECT_DEPLOYMENT_CALLTOUCH or "calltouch-loader-clickhouse/calltouch-loader-clickhouse"
PREFECT_DEPLOYMENT_CALLTOUCH_SINGLE = settings.PREFECT_DEPLOYMENT_CALLTOUCH_SINGLE or PREFECT_DEPLOYMENT_CALLTOUCH
...
async def process_upload_ct_tdelta(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not site_id:
            await message.answer("❌ Сначала укажите Site ID клиента.", reply_markup=get_kb_calltouch())
            return
        ...
        await message.answer(f"❌ Ошибка: {e}", reply_markup=get_kb_calltouch())
