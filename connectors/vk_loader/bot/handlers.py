...
from .keyboards import get_kb_vk, get_kb_vk_upload_only
...
async def _get_vk_menu(user_id: int):
    if await is_alpha_user(user_id):
        return get_kb_vk_upload_only()
    return get_kb_vk()
...
async def handle_vk_agencies(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        await callback_query.message.answer(
            "Выберите действие:",
            reply_markup=await _get_vk_menu(callback_query.from_user.id)
        )
...
async def process_add_vk_agency(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            reply_markup=await _get_vk_menu(message.from_user.id),
            parse_mode="HTML"
        )
    ...
    try:
        ...
        asyncio.create_task(
            wait_for_prefect_flow_run(
                run_id,
                deployment_name=PREFECT_DEPLOYMENT_VK,
                notify_message=callback_query.message,
                poll_interval=20,
