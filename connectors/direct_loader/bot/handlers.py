...
from .keyboards import get_kb_direct, get_kb_direct_upload_only
...
async def _get_direct_menu(user_id: int):
    if await is_alpha_user(user_id):
        return get_kb_direct_upload_only()
    return get_kb_direct()
...
async def handle_yd_clients(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        await callback_query.message.answer(
            "Выберите действие:",
            reply_markup=await _get_direct_menu(callback_query.from_user.id)
        )
...
async def process_add_yd_client(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if token_type == DIRECT_TYPE_AGENCY_TOKEN:
            ...
            await message.answer(
                f"✅ Клиент '{normalized_login}' добавлен (container: {normalized_container}).",
                reply_markup=await _get_direct_menu(message.from_user.id),
            )
