...
class AsyncCallibriDatabase:
    """
    Callibri loader wrapper for ClickHouse operations.

    Configuration:
    - Uses Accesses table with service='callibri'
    - login: site_id (as string)
    - token: Callibri API token
    - container: account name/description (optional)
    - type: unused (can be None)
    """

    def __init__(self, *, database: str | None = None):
        target_db = database or CLICKHOUSE_DB_CALLIBRI or CLICKHOUSE_DATABASE
    ...
    async def init_db(self):
        """Create the Callibri database if it does not exist."""
        await self._data_db.ensure_db()

    async def add_client_to_callibri_config(
        self,
        site_id: int,
        token: str,
        ...
        """
        Add a Callibri client to the Accesses table.

        Args:
            site_id: Callibri site ID (stored as login)
            token: API token
    ...
    async def delete_client_by_site_id(self, site_id: int):
        """Delete a Callibri client from the Accesses table."""
        site_id_str = str(site_id)
        await self._access_db.delete_access(site_id_str, service_type="callibri")

    async def get_callibri_config_data(self) -> pd.DataFrame:
        """
        Retrieve all Callibri clients from the Accesses table.

        Returns:
            DataFrame with columns: site_id, token, account
        """
        access_rows = await self._access_db.fetch_access_rows(service_type="callibri", include_null_type=True)
    ...
    async def reset_database(self) -> dict[str, int]:
        """
        Reset the Callibri database - truncate all tables.
    ...
    async def get_stale_clients(self, days_threshold: int = 30):
        """
        Get list of Callibri clients with stale data (oldest date > threshold days old).
