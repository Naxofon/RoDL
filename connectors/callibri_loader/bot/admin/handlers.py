...
from connectors.callibri_loader.bot.config import PREFECT_DEPLOYMENT_CALLIBRI
...
router_callibri_admin = Router()


class CallibriAdminUploadStates(StatesGroup):
...
@router_callibri_admin.callback_query(lambda c: c.data == "admin_upload_all_callibri")
async def handle_upload_all_callibri(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.answer(
        "📍 Админ-панель → Выгрузка Callibri\n\n"
        "💬 <i>Выгрузка Callibri для всех клиентов</i>\n\n"
    ...
    await state.set_state(CallibriAdminUploadStates.waiting_tdelta)


@router_callibri_admin.message(CallibriAdminUploadStates.waiting_tdelta)
async def process_callibri_tdelta(message: types.Message, state: FSMContext):
    try:
        text = await _require_text(message)
        if text is None:
            return
        tdelta = _parse_lookback(text)
        await message.answer(f"🔄 Запускаю выгрузку Callibri за последние {tdelta} дней.")
        ...
        async def run():
            ...
            if not ran_via_prefect:
                await message.answer(
                    "⏹ Запуск Callibri отменён (не удалось создать Prefect run).",
                    reply_markup=get_kb_admin(),
                )
        ...
        await message.answer(f"❌ Ошибка запуска Callibri: {_format_error(exc)}.", reply_markup=get_kb_admin())
    finally:
        await state.clear()
