...
from connectors.metrika_loader.bot.config import PREFECT_DEPLOYMENT_METRIKA_ALL
...
router_metrika_admin = Router()


class MetrikaAdminUploadStates(StatesGroup):
...
@router_metrika_admin.callback_query(lambda c: c.data == "admin_upload_all_metrika")
async def handle_upload_all_metrika(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(MetrikaAdminUploadStates.waiting_lookback)


@router_metrika_admin.message(MetrikaAdminUploadStates.waiting_lookback)
async def process_metrika_lookback(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"🔄 Запускаю проверку изменений за последние {lookback_days} дней.")
        ...
        await message.answer(
            f"❌ Ошибка запуска выгрузки: {_format_error(exc)}.",
            reply_markup=get_kb_admin(),
        )
    finally:
        await state.clear()
