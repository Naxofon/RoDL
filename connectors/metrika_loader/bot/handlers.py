...
from .keyboards import get_kb_metrika, get_kb_metrika_upload_only
from services.prefect_client import (
    trigger_prefect_run,
    wait_for_prefect_flow_run,
)
from services.user_roles import get_user_role, is_alpha_user
from connectors.metrika_loader.bot.config import (
...
async def _get_metrika_menu(user_id: int):
    if await is_alpha_user(user_id):
        return get_kb_metrika_upload_only()
    return get_kb_metrika()


async def _deny_if_alpha(callback_query: CallbackQuery) -> bool:
...
async def handle_ym_clients(callback_query: types.CallbackQuery):
    ...
    if df.empty:
        await callback_query.message.answer(
            "⚠ Клиенты не найдены",
            reply_markup=await _get_metrika_menu(callback_query.from_user.id),
        )
    else:
...
async def process_add_ym_client(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            reply_markup=await _get_metrika_menu(message.from_user.id),
        )
...
async def process_remove_ym_client(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("✅ Клиент успешно удалён", reply_markup=await _get_metrika_menu(message.from_user.id))
    except ValueError:
